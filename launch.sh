...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "workflowPurgeAPI-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name workflowPurgeAPI-service apps/workflowPurgeAPI-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app workflowPurgeAPI-service:1.0-SNAPSHOT
    oc expose svc/workflowPurgeAPI-service
else

	echo "Launching the application locally..."
	pattern="workflowPurgeAPI-service"
	files=( $pattern )
	cd ${files[0]}
